...
<!-- JS libraries -->
    ...
    <script src="javascript/reportdata_na.js"></script>
    ...
    <script>
        ...
        require([
            ...
            "dojo/domReady!"
             ...
             function (esriConfig, esriRequest,all) {
                ...
                var bunit = "";
                var coords = "";
				//RW add ptitle if passed in 8/12/24
				var customtitle = "";
			   ...
			   if (useStaticData) {
                    ...
                    var req0 = new esriRequest(soeurldemog, {
                        query: content,
                        responseType: "json",
                ...
                if (useStaticData) {
                    ...
                    var req1 = esriRequest(soeurlejmain, {
                        query: content,
                        responseType: "json",
                        callbackParamName: "callback"						
                    })
                ...
                if (useStaticData) {
                    ...
                    var req2 = esriRequest(soeurlejextra, {
                        query: content,
                        responseType: "json",
                        callbackParamName: "callback"
                    })
			...
			//demog
			//set flag but this will be caught in EJ call as well, no centroids
			if (result0.data.errInfo != null ){
				errorFlag = true;				
			}
			else if (result0.data.statGroupList[0].statList[0] && result0.data.statGroupList[0].statList[0].value == "0"){
			//catch second demog error, case of centroids but 0 pop
				errorFlag = true;	
				//set error here, move to SOE?
				//Note 6/27/23 RW:
                    ...
                    let no_report_msg_html = "<tr style='max-height: 115px'><td><p id='cannot-report-msg'>" + no_report_msg + "</p></td></tr>";
			...
			//main
			if (result1.data.message != null ){
				errorFlag = true;
				
				//RW 2/21/23 show if error if over or under limit from EJ service instead of demog test. 
				   //testing demog return only catches under limit, not over limit.
				   //if EJ return has error message show it
				   let no_report_msg = result1.data.message;
				   let no_report_msg_html = "<tr style='max-height: 115px'><td><p id='cannot-report-msg'>" + no_report_msg + "</p></td></tr>";
                    if (result1.data.message) {
                        if (result1.data.messageType) {                           
                            document.getElementById('geo-info-bar').innerHTML = no_report_msg_html;  
							//errorMode = true;	
							//setAllNull();
                            //return false;
			...
			useStaticData = true;
					...
					processResultDemog(result0);
					processResultEJMain(result1);
					processResultEJExtra(result2);
			}
                ...
                function processResultDemog(response) {
				 ...
				 //var str0 = document.getElementsByTagName("body");
				 //var str = str0[0];
				 //var res = str.innerHTML.replaceAll("percent", "");
				 //res = res.replaceAll("%", " ");
				 //str.innerHTML = res;
				 
    // var x = str.getElementsByTagName("div");
   // for (var i = 0; i < x.length; i++) {
       // var res = x[i].innerHTML.replace("percent", "");
		//res = x[i].innerHTML.replace("%", "");
       /// x[i].innerHTML = res;
    // }
					...
					//RW 1/29/24 - only do error check on EJ main
					/*
                    let no_report_msg = "The area is too small or sparsely populated, or these data are not available in the national dataset. " + 
                              "Cannot generate an EJScreen chart or report."
                    let no_report_msg_html = "<tr style='max-height: 115px'><td><p id='cannot-report-msg'>" + no_report_msg + "</p></td></tr>";
					...
					*/
					...
					//if error input area div is removed, don't write
					if (document.getElementById("inputAreaMiles")){
						document.getElementById("inputAreaMiles").innerHTML = Number(resultDEMOG.inputAreaMiles);
					}

					// if (document.getElementById("LOCATIONSTR")){
                ...
                function processResultEJMain(response) {
                    ...
                    Object.entries(ejmainHash).forEach(entry => {
						...
						//RW 8/12/24 add custom title is passed in
						if (customtitle != ""){
					...
					//RW 2/27/24 only format if live data else gives 'NaN' in table
        ...
        <div class="print-page">
            ...
            ">
                ...
                <p style="font-family: Oswald, Arial, sans-serif; font-weight: 300; font-size: 16px; margin-bottom: -2px; padding-left: 20px;">
                    ...
                    education, percent limited English speaking, and percent low life expectancy with a single 
